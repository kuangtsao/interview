...
def uat(bot,jenkins_robot,version,chat_id):
    bot.send_message(chat_id,"開始更新 B 組 UAT 遊戲大廳")
    update_lobby = 'https://jenkins.fromer.url/job/B_GAMES_LOBBY_UPDATE/buildWithParameters'
    parameters = { 'enviorment': 'UAT', 'version': version, 'bot': jenkins_robot['bot_to']}
    requests.post(update_lobby, data=parameters, auth=jenkins_robot['config'])


def demo(bot,jenkins_robot,chat_id):
    bot.send_message(chat_id,"開始更新 B 組 DEMO 遊戲大廳")
    update_lobby = 'https://jenkins.fromer.url/job/B_GAMES_LOBBY_UPDATE/buildWithParameters'
    parameters = { 'enviorment': 'DEMO', 'version': version, 'bot': jenkins_robot['bot_to']}
    requests.post(update_lobby, data=parameters, auth=jenkins_robot['config'])

def prd(bot,jenkins_robot,mode,chat_id):
    bot.send_message(chat_id,"開始更新 B 組 PRD 遊戲大廳")
    update_lobby = 'https://jenkins.fromer.url/job/B_GAMES_LOBBY_UPDATE/buildWithParameters'
