...
if [ $ENVIORMENT == "UAT" ]; then
  ...
  if [ $OPTION == "all" ]; then
    GAMES="/data/YG/stop_game_bylist.sh all_games.txt"
    NOTIFY_MESSAGE="預計關閉 $ENVIORMENT 環境所有遊戲"
    curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$NOTIFY_MESSAGE"
  elif [ $OPTION == "partial" ]; then
    GAMES="/data/YG/stop_game_bylist.sh"
    NOTIFY_MESSAGE="預計關閉 $ENVIORMENT 環境以下遊戲："
  ...
  if [ $OPTION == "all" ]; then
    ...
    GAMES="cd /data/YG && /data/YG/stop_game_bylist.sh"
    NOTIFY_MESSAGE="預計關閉 $ENVIORMENT 環境以下遊戲："
    GAMES_LIST_CONTENT=$(ssh -i $IDENTITY_FILE_PATH $USER@$HOST cat $SVN_GAMES_LIST)
    curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$NOTIFY_MESSAGE"
    curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$GAMES_LIST_CONTENT"
  ...
  ssh -i $IDENTITY_FILE_PATH $USER@$HOST $DEPLOY_GAMES_LIST
  if [ $OPTION == "all" ]; then
    GAMES="cd /data/YG && /data/YG/stop_game_bylist.sh all_games.txt"
    NOTIFY_MESSAGE="預計關閉 $ENVIORMENT 環境所有遊戲"
    curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$NOTIFY_MESSAGE"
  elif [ $OPTION == "partial" ]; then
    GAMES="cd /data/YG && /data/YG/stop_game_bylist.sh"
    ...
    GAMES_LIST_CONTENT=$(ssh -i $IDENTITY_FILE_PATH $USER@$HOST cat $SVN_GAMES_LIST)
    curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$NOTIFY_MESSAGE"
    curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$GAMES_LIST"
  else
    echo "no other option"
    exit 0
  fi
...
echo "Step 2: 關閉 $ENVIORMENT 環境的所有 YG 遊戲"
ssh -i $IDENTITY_FILE_PATH $USER@$HOST $GAMES
curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="已關閉 $ENVIORMENT 環境 GAMES 服務。"
