...
VERSION=$2
BOT=$3
UPDATE_FILE_PATH="/data/svn/yg001/code/backend/YGGameResult/YGGameResult-$ENVIORMENT-$VERSION.war"
...
if [ $ENVIORMENT == "TEST" ]; then
  ...
  DEPLOY_PATH="/data/tomcat-gameresult/webapps/ROOT"
  UPDATE_FILE_PATH="/data/svn/yg001/code/backend/YGGameResult/YGGameResult-$ENVIORMENT-$VERSION.warr"
  BINARY_PATH="/data/tomcat-gameresult/bin"
...
if [ $? != 0 ]; then
  ...
  curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$JOB_NAME：開始更新gameresult api $VERSION 版"
fi

echo "Step 1: 上傳需更新版本"
CP_FILE="cp -a $UPDATE_FILE_PATH /tmp/YGGameResult.war"
...
echo "Step 2: 關閉 $ENVIORMENT 環境的 gameresult api"
SHUTDOWN_TOMCAT="$BINARY_PATH/catalina.sh stop"
ssh -i $IDENTITY_FILE_PATH $USER@$HOST $SHUTDOWN_TOMCAT 
curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="已關閉 $ENVIORMENT 環境 gameresult api"

echo "Step 3: 備份舊檔案"
BACKUP_PREVIOUS_VERSION="cp $DEPLOY_PATH/YGGameResult.war /data/backup/YGGameResult.war_$DATE"
ssh -i $IDENTITY_FILE_PATH $USER@$HOST $BACKUP_PREVIOUS_VERSION 
curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$JOB_NAME：舊檔案已備份為 /data/backup/YGGameResult.war_$DATE"

echo "Step 4: 部署 $ENVIORMENT 環境 gameresult api $VERSION"
REMOVE_OLD_VERSION="rm -rf $DEPLOY_PATH/*"
COPY_UPLOAD_FILE="cp /tmp/YGGameResult.war $DEPLOY_PATH"
DEPLOY_NEW_VERSION="cd $DEPLOY_PATH && jar -xvf YGGameResult.war"
...
echo "Step 5: 開啟 $ENVIORMENT 環境 gameresult api"
STARTUP_TOMCAT="$BINARY_PATH/catalina.sh start"
ssh -i $IDENTITY_FILE_PATH $USER@$HOST $STARTUP_TOMCAT 
curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="已開啟 $ENVIORMENT 環境 gameresult api"

echo "Step 6: 清理 /tmp"
REMOVE_TMP_JAR="rm -f /tmp/YGGameResult.war"
