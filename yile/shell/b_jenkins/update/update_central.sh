...
if [ $ENVIORMENT == "UAT" ]; then
  ...
  EXECUTE_UPDATE_CENTRAL="cd /data/YG && /data/YG/03_UAT_update_Central.sh"
  ...
  EXECUTE_UPDATE_CENTRAL="cd /data/YG && /data/YG/03_DEMO_update_Central.sh"
  ...
  EXECUTE_UPDATE_CENTRAL="cd /data/YG && /data/YG/03_Prd_update_Central.sh"
...
if [ $? != 0 ]; then
  ...
  curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$JOB_NAME；開始更新 YG central $VERSION 版"
fi
...
echo "Step 2: 執行更新 central "
curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$JOB_NAME：開始更新 $ENVIORMENT central"
ssh -i $IDENTITY_FILE_PATH $USER@$HOST $EXECUTE_UPDATE_CENTRAL
curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$JOB_NAME：$ENVIORMENT central 更新結束"

#echo "刪除 /tmp/YGCentral"
#REMOVE_TMP_CENTRAL="rm -f /tmp/YGCentral"
#ssh -i $IDENTITY_FILE_PATH $USER@$HOST $REMOVE_TMP_CENTRAL
