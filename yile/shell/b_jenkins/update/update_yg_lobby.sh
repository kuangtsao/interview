...
#./update_yg_lobby.sh enviorment version
...
if [ $ENVIORMENT == "UAT" ]; then
  ...
  BACKUP_OLD_VERSION="cp /data/YG/YGLobby/release/YGLobby /data/YG/YGLobby/backup/YGLobby_$DATE"
  UPDATE_NEW_VERSION="cp /tmp/YGLobby /data/YG/YGLobby/release/YGLobby"
  REMOVE_TMP_LOBBY="rm -f /tmp/YGLobby"
...
curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$JOB_NAME：開始更新 $ENVIORMENT 環境 YG 總大廳"
...
if [ $? != 0 ]; then
  ...
  curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$JOB_NAME；開始更新 YG 總大廳 $VERSION 版"
fi
...
echo "Step2: 備份 YG 總大廳"
ssh -i $IDENTITY_FILE_PATH $USER@$HOST $BACKUP_OLD_VERSION
echo "Step3: 更新 YG 總大廳"
ssh -i $IDENTITY_FILE_PATH $USER@$HOST $UPDATE_NEW_VERSION

#echo "刪除 /tmp/YGLobby"
#ssh -i $IDENTITY_FILE_PATH $USER@$HOST $REMOVE_TMP_LOBBY

curl -s -X POST "$BOT_TOKEN" -d chat_id="$CHAT_ID" -d text="$JOB_NAME：$ENVIORMENT 環境 YG 總大廳更新完畢"
