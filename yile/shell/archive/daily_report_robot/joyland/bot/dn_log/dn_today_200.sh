...
for((i=0; i<=23; i++))
do
        x=$(cat "$logpatch/DN_Statistics_Room_200.log.txt" | awk 'BEGIN {FS="玩了"} NR=='$i'+9 {print ($2)-0}')
        atotal=$(($atotal+$x))
done
...
asystowii=$(cat "$logpatch/DN_Statistics_Room_200.log.txt" |awk 'BEGIN {FS="玩家總贏="}; NR==8{print ($2)-0}')
